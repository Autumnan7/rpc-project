...
void TcpServer::start_multi(std::string_view ip, int port)
{
    ...
    // 清空并预留空间，避免多次扩容
    _multi_listen_fd.clear();
    // reserve 修改capacity，不改变size，后续 emplace_back 会在预留空间上直接构造对象，避免多次内存分配和对象移动，提高性能
    _multi_listen_fd.reserve(tCnt);
    ...
    for (int i = 0; i < tCnt; ++i)
    {
        ...
        {
            LOG_ERROR("create socket failed on core %2d, maybe reach fd limit?", i);
            return;
        }

        LOG_INFO("tcpserver listen fd ready on core %2d", i);
        ...
        {
            LOG_ERROR("multi server bind error on core %2d", i);
