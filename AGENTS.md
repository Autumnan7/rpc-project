...
> 测试时间：2026-06-01
> 测试工具：rpc_benchmark（1000/10000 协程长连接，10 秒满载压测）
...
- **非绑定模式 (bind_thread=false)**：业务协程走全局负载均衡 (`MIN_EVENT_FIRST`)，由 `ProcessorSelector` 选最空闲的核
...
| **P50 延迟** | 0.76 ms | 0.42 ms | 非绑定更低 |
| **P90 延迟** | 11.20 ms | 21.84 ms | 绑定 **-48.6%** |
| **P95 延迟** | 15.16 ms | 24.72 ms | 绑定 **-38.9%** |
| **P99 延迟** | 24.76 ms | 31.66 ms | 绑定 **-21.9%** |
| **P99.9 延迟** | 38.38 ms | 41.26 ms | 绑定 **-7.0%** |
| **失败数** | 0 | 0 | — |
...
| **P50 延迟** | 1.10 ms | 0.72 ms | 非绑定更低 |
| **P90 延迟** | 171.03 ms | 238.87 ms | 绑定 **-28.4%** |
| **P95 延迟** | 187.03 ms | 299.36 ms | 绑定 **-37.5%** |
| **P99 延迟** | 224.01 ms | 354.11 ms | 绑定 **-36.7%** |
| **P99.9 延迟** | 298.62 ms | 409.27 ms | 绑定 **-27.0%** |
| **失败数** | 0 | 0 | — |

#### 分析

**绑定模式优势**：两种并发度下绑定模式全面胜出。1K时QPS +14.2%，10K时QPS +12.8%。尾延迟差距更显著：10K时P95绑定模式比非绑定低37.5%（187ms vs 299ms），P99低36.7%（224ms vs 354ms）。连接在accept核上完成握手，业务协程留在同一核处理，保住L1/L2缓存亲和性，避免跨核缓存失效。

**非绑定模式现象**：两种并发度下P50都更低（1K: 0.42 vs 0.76, 10K: 0.72 vs 1.10），但这是虚假优势——全局负载均衡把少量请求快速分到空闲核，同时把大量请求堆积在繁忙核上，造成长尾拥堵。10K时这个效应更极端：非绑定P90 238ms是绑定171ms的1.4倍，P99 354ms是绑定224ms的1.6倍。

**并发度对比**：1K→10K时，绑定模式QPS仅从277K降到244K（-12.1%），而非绑定从242K降到216K（-10.7%）。两者吞吐量衰减幅度相近，但尾延迟差距随并发度放大——非绑定在10K下的P95/P99接近绑定的1.6倍，说明全局负载均衡在高并发下的选核开销和核间竞争问题被进一步放大。
